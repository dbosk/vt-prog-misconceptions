...
Variation theory offers several further principles that inform our analysis
\parencite{NCOL}. Learning transfers more readily when the object of learning is
met in varied settings rather than only the one in which it was taught --- as
when a skill learned in one place is tested in another. Grouping helps too:
teaching similar concepts together, so that their differences can be contrasted,
leads to more learning than teaching each in isolation. And directing attention
to one attribute at a time, rather than to the whole concept at once, supports
the discernment of each critical aspect in turn.
