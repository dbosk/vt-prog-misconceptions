...
A simple example illustrates what such differences in experience look like
\autocite[p.~24]{NCOL}. Asked how many fingers she has on her left hand, a
six-year-old child answers \enquote{five}; asked how many she has on her right
hand, she answers \enquote{ten}. The child is not so much making an arithmetic
error as experiencing the question differently from the adult, and it is worth
asking what \enquote{ten} means to her.
...
\Textcite{Neuman1987} studied precisely this kind of difference. Numbers have
three properties that arithmetic requires: they are \emph{ordinal} (first,
second, third, \dots), they are \emph{cardinal} (one, two, three, \dots), and
they stand in \emph{part--whole} relations
(\(4 = 2 + 2 = 1 + 3 = 3 + 1\)). The child who answered \enquote{ten} had
discerned only the ordinal property: counting on across both hands, the tenth
finger is simply the one that comes tenth in the sequence. Different children
have discerned different properties --- different aspects of what a number is
--- and phenomenography is the means by which we can discover which aspects
those are. In this article we use the literature on programming misconceptions
as our phenomenographic observations: each documented misconception records an
aspect that some learners have not yet discerned.
