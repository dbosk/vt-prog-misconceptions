...
is invariant, and we have variation on the shape of the figures.
...
In this example the critical aspect is colour, with green as the critical
feature, contrasted against blue; the non-critical aspects are shape and the
order of the pairs, whose features here are the circle and the square.
...
In teaching we often lean on induction alone --- generalisation without a
preceding contrast --- as when we point students to \enquote{good theses from
previous years}, or show one example of a linear function after another.
Without the contrast the critical aspect never comes into focus, since nothing
varies against it.
...
Evidence from other domains is consistent with this. In electrocardiogram (ECG)
interpretation, \textcite{hatala2003practice} found that medical students who
practised on contrasting examples drawn from several diagnostic categories mixed
together reached markedly higher diagnostic accuracy on novel cases (\(46\%\))
than students who practised each category in a separate block (\(30\%\)).
