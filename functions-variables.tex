...
\mode<presentation>{%
\begin{frame}
  \begin{block}{Misconception\autocite{%
          Kohn2017VariableEvaluation,%
          Plass2015Variables,%
          Doukakis2007%
        }}
...
\mode<presentation>{%
\begin{frame}
  \begin{block}{Misconception\autocite{Kohn2017VariableEvaluation}}
    \begin{itemize}
...
A third documented misconception about what a variable holds concerns how many
values it holds: \textcite{Sorva2012Dissertation} catalogues, in his inventory
of documented misconceptions, that
\textcquote[Table~A.1, p.~359]{Sorva2012Dissertation}{A variable can hold
multiple values at a time / \enquote{remembers} old values}, crediting among
others \textcite{Doukakis2007}.
A student holding it expects an assignment to \emph{add} to the variable, so
that earlier values remain retrievable.

\mode<presentation>{%
\begin{frame}
  \begin{block}{Misconception\autocite{Sorva2012Dissertation,Doukakis2007}}
    \begin{itemize}
      \item A variable can hold multiple values at a time, or
        \enquote{remembers} its old values.
    \end{itemize}
  \end{block}
\end{frame}%
}

The critical aspect is statefulness in its destructive guise: an assignment
\emph{replaces} the variable's value.
A minimal contrast keeps the printed variable invariant and varies only
whether it is assigned once or twice; the left program prints
\mintinline{python}{5}, the right prints \mintinline{python}{3} alone ---
not both values, and not the first.
...
The statefulness pattern below then generalises this: each assignment
happens at its line, using the values current at that moment, and what a
variable held before is gone.
...
\begin{enumerate}
  \item<+> That variables and functions are interconnected and should be 
    ...
    contrast them \autocite[\cf][Ch~6, pp~167--168]{NCOL}.
...
\autocite{Kohn2017VariableEvaluation,Plass2015Variables,Doukakis2007} is 
...
The remaining aspects of functions follow the same approach. We treat the order
of arguments and the scope of identifiers below, since both are tied to
documented misconceptions; further aspects, such as default and keyword
arguments, can be taught with the same contrast--generalisation--fusion patterns
once these are in place.

\subsubsection{Order of arguments}

\begin{frame}[fragile]\label{vtprogord}
  \begin{example}
    A simple function that greets the user.
    \lstexample{python}{examples/greet.py}
  \end{example}

  \begin{block}{Output}
    \runpython{examples/greet.py}
  \end{block}
\end{frame}

\begin{frame}[fragile]\label{vtprogordcon}
  \begin{example}[Constrast argument order]
    The same function as in \cref{vtprogord}, but with the arguments in the 
    wrong order.
    \lstexample[highlightlines={1,7}]{python}{examples/order-contrast.py}
  \end{example}

  \begin{block}{Output}
    \runpython{examples/order-contrast.py}
  \end{block}
\end{frame}

For generalisation we can return to the earlier examples: the order of the
arguments was correct in each of them.
...
\begin{frame}[fragile]\label{vtprogid}
  \begin{example}
    The same function as in \cref{vtprogord}.
    \lstexample{python}{examples/greet.py}
  \end{example}
...
\begin{frame}[fragile]\label{vtprogidcon}
  \begin{example}[Contrast identifiers in different scopes]
    The same function as in \cref{vtprogid}, but with the variables of the 
    \mintinline{python}{main} function named differently from the arguments of 
    the \mintinline{python}{greet} function.
    \lstexample[highlightlines=5-7]{python}{examples/scope-contrast.py}
  \end{example}

  \begin{block}{Output}
    \runpython{examples/scope-contrast.py}
  \end{block}
\end{frame}

\begin{frame}[fragile]
  \begin{example}[Generalize identifiers in different scopes]
    \lstexample[highlightlines=5]{python}{examples/scope-contrast-more.py}
  \end{example}

  \begin{block}{Output}
    \runpython{examples/scope-contrast-more.py}
  \end{block}
\end{frame}
...
\autocite{MisconceptionsSurvey2017,Sleeman1984}. For example, if a student 
...
examples where the variable names are disconnected from the purpose --- here a
variable named \mintinline{python}{maximum} that in fact computes the
\emph{minimum}.
...
\hfill
    \begin{minipage}[t]{0.45\columnwidth}
        \begin{minted}[highlightcolor=light-green, highlightlines={2,7}]{python}
             def example(values):
                maximum = values[0]
                for value in values:
                    if value < maximum:
    ...
    \begin{minipage}[t]{0.45\columnwidth}
        \begin{minted}{python}
             def example(hi):
                ...
                for i in y:
                    if i.property2 > 18:
                        print(f"Hi {i.property1}! You are an adult.")
        \end{minted}
    \end{minipage}
\hfill
...
different statements, by printing the output of
\mintinline{python}{x == y} and \mintinline{python}{x = y} separately.
Both \mintinline{python}{x} and \mintinline{python}{y} can be pre-defined,
to avoid the programme throwing an error. However, in the contrast we could
also include the example where an error is thrown, this to
\emph{contrast} even further.
...
\hfill
      ...
      The last pattern, \emph{Fusion}, should consist of both
variable-definitions and a place where the variables are then used in a
comparison-statement, so that the student must tell assignment
(\mintinline{python}{=}) from comparison (\mintinline{python}{==}) within a
single program.
...
invariant and variant with each other, since the contrasting case throws an
error: there is no running program in which both readings coexist, so fusion
does not apply here.
\end{enumerate}


Since Python is an interpreted language, the placement of the definition of
a function matters, something that differs from a compiled language. This gives
room for a misconception among students who have learned to code in, for
example, Java, where the definition of a function can be placed below a call of
the function. We include this misconception with a caveat: it concerns students
transferring from another language rather than complete novices, and so sits at
the edge of our scope; we keep it because students who met programming in high
school commonly carry it.

The object of learning is that, in Python, a function must be defined before the
line that calls it. The critical aspect is the top-to-bottom execution order, so
a single contrast suffices: holding the function and the call invariant, we vary
only their order. With the definition first the program runs; with the call
first it raises a \mintinline{python}{NameError}, because the name does not yet
exist when the call is reached.
...
A related misconception concerns the call itself: students may define a function
and then never call it, expecting the definition alone to run its
body.\label{defined-not-called}
\textcite{Bosse2021Antipatterns} catalogue this \emph{function created but not
called} antipattern: \textcquote{Bosse2021Antipatterns}{Creating it but not
calling it will cause that any task that the program needs to perform will not be
executed}. The object of learning is that \emph{defining} a function and
\emph{invoking} it are two separate acts --- the body runs only at the call. A
single contrast, holding the function invariant and varying only whether the call
is present, makes this discernible: on the left nothing is printed, because the
body is never reached; on the right the call runs it.
...
arguments 
come from and go to \autocite{Ragonis2005OOP}.
...
expecting the function to return it by default \autocite{
Kurvinen2016,KumarVeerasamy2016}.

    \item Believe that a print-statement at the end of a function will act 
as a return statement \autocite{MisconceptionsSurvey2017}.
...
The trouble with returning does not end once students write a
return-statement: \textcite{KumarVeerasamy2016} found that even among the
students who understood functions,
\textcquote[manuscript p.~12]{KumarVeerasamy2016}{8\% of the students passed the wrong
argument/value as the return value}.
The critical aspect is \emph{which} of the function's names the return
statement hands back, so the contrast keeps the function's whole computation
invariant and varies only the returned variable: the left program assigns
\mintinline{python}{6} to \mintinline{python}{x} (the sum), the right
\mintinline{python}{2.0} (the average the name promises).
...
The generalisation and fusion of the first pattern above then apply
unchanged, since their examples already return the value their names
promise.
...
A final aspect of the return statement is easily missed altogether: that it
\emph{ends} the function.
\textcite{Esche2025Return}, analysing over 3000 free-text descriptions of
functions by CS1 students, reports:
\textcquote{Esche2025Return}{The keyword \enquote{return} itself was rarely
mentioned explicitly}, and further:
\textcquote{Esche2025Return}{The aspect that the keyword \enquote{return} ends
the function was even less often explicitly mentioned}; the study also
surfaced, in its Java setting, the new misconception that
\textcquote{Esche2025Return}{\enquote{return} and \enquote{void} are mutually
exclusive}.
An aspect the students do not mention is an aspect we cannot assume they
discern.

\mode<presentation>{%
\begin{frame}
  \begin{block}{Misconception\autocite{Esche2025Return}}
    \begin{itemize}
      \item Novices rarely discern that \mintinline{python}{return}
        \emph{ends} the function --- they treat it as only handing back a
        value.
    \end{itemize}
  \end{block}
\end{frame}%
...
The critical aspect is that an executed return statement terminates the call
then and there: statements after it never run.
The contrast keeps the function's two statements invariant and varies only
their order --- calling the left function prints only \mintinline{python}{done}
(the print-statement is never reached), the right prints
\mintinline{python}{working} and then \mintinline{python}{done}.
...
To generalise, we keep the critical value invariant --- the return ends the
call the moment it executes --- and vary where it sits: at the end of the
body, inside a conditional, inside a loop.
The last of these is also a fusion, since the student must attend to the
loop, the condition and the early exit simultaneously: the function below
returns from \emph{inside} the loop at the first negative value, so for
\mintinline{python}{[4, -1, -5]} it returns \mintinline{python}{-1}, not
\mintinline{python}{-5} --- the loop does not run to completion.
...
that 
the computer will solve this as an equation \autocite{Plass2015Variables}. 
...
is the 
variable, 
and that the \mintinline{python}{x} to the right only holds a value. The
augmented form \mintinline{python}{x += 1} avoids inviting this reading
altogether, since it does not repeat the name and so cannot be mistaken for
an equation.

The object of learning is that the \mintinline{python}{=} of programming is
\emph{assignment}, not mathematical equality. The critical aspect is that
assignment is directional and time-ordered: it computes the right-hand side from
the \emph{current} values and stores the result in the name on the left. Read
that way, \mintinline{python}{x = x + 1} is unremarkable, whereas read as an
equation it is unsolvable. We make this aspect discernible by holding the
expression invariant and varying only whether the symbol assigns or compares.

\begin{description}
  \item[Contrast] The same expression \mintinline{python}{x + 1}, varying only
    the operator. On the left \mintinline{python}{=} recomputes and rebinds, so
    \mintinline{python}{x} becomes \mintinline{python}{6}; on the right
    \mintinline{python}{==} tests equality and is always
    \mintinline{python}{False} --- which is exactly the contradiction a student
    sees when reading the assignment as an equation.
  ...
  \item[Generalisation] Holding the rule invariant --- the right-hand side is
    computed from the current value and stored back --- we vary the operation,
    including the augmented form \mintinline{python}{x += 1}, which does not
    repeat the name and so does not invite the equation reading.
  ...
  \item[Fusion] Finally we fuse the two critical aspects --- that
    \mintinline{python}{=} assigns rather than compares, and that each assignment
    acts on the value left by the previous line --- in a single trace that uses
    both \mintinline{python}{=} and \mintinline{python}{==}. To predict the
    output the student must read lines~1--2 as directional updates (so
    \mintinline{python}{x} becomes \mintinline{python}{6}) and line~3 as a
    \emph{test} of the resulting value, which prints \mintinline{python}{True}.
