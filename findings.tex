...
in the right order, without ever being invoked. This is the \emph{function
created but not called} antipattern that \textcite{Bosse2021Antipatterns} also
catalogue, so our observation corroborates the literature here. The critical
aspect --- that definition and invocation are distinct --- and the contrast that
makes it discernible, holding the definition invariant and varying only whether
the call is present, are given with the catalogue's treatment in
\cref{defined-not-called}.
