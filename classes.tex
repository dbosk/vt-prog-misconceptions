...
\ltnote{%
  Relevant misconceptions (cited in the body): the class seen as a collection of
  objects or strictly as a database, with the class's methods able to change,
  add, and delete its instances.
  \textbf{What varies} (contrast): only whether the type is built-in or
  user-defined.
  \textbf{What stays invariant}: the act of calling a type to create an object,
  so attention falls on \enquote{a class is a template, like the types you
  already use}.%
}
...
\ltnote{%
  Relevant misconceptions (cited in the body): that an object's attributes are
  its identifier, so two objects cannot share attribute values, and difficulty
  grasping that each object has its own memory space.
  \textbf{What varies} (contrast): only whether the two names denote two objects
  or one.
  \textbf{What stays invariant}: the attribute values, so the difference in
  behaviour isolates object identity from attribute equality.%
}
