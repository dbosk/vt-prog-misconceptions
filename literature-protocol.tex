...
\autocite{scholar}, which queries several bibliographic databases at once,
...
exactly one --- the most-cited survey \autocite{MisconceptionsSurvey2017} ---
...
together with citation snowballing (\cref{app:litsnowball}) --- rather than
either replacing the other.

\subsection{Verification and provenance}

Following the find--verify--record discipline, every reference cited in the
catalogue carries a
...
\label{app:litsnowball}

The snowballing step deferred by the initial run was carried out later, in one
generation over the catalogue's own primary sources. The seed set was the
eleven DOI-resolvable primary misconception studies cited in the catalogue
(two further primaries, Sleeman's report and Plass's thesis, lack DOIs);
backward snowballing followed every work a seed references, and forward
snowballing every work citing a seed (up to 200 per seed), both through the
OpenAlex API. This produced 787 distinct candidates, of which 24 were already
in the corpus --- the bibliographies, the systematic phase's retrieved set and
its decision log. A keyword screen for topic left 401, a stricter screen
requiring both the population (novice, introductory, CS1) and an error- or
misconception-bearing title left 51, and these were hand-screened against the
protocol's inclusion criterion. Fifteen were kept; the 36 exclusions are
recorded with per-paper reasons alongside the script
(\texttt{literature-review/}).

The kept set validates the coverage hypothesis of \cref{app:litcomparison}:
the highest-cited recoveries are precisely the classic primaries that the
inventory literature attributes --- \textcite{Pea1986Superbug} among them,
which we integrate into the catalogue (\cref{problem-solving}) --- alongside
recent successors of our seeds. The remaining kept papers await integration;
they are tracked in the repository's issue tracker.
...
not exhaustive; the citation snowball (\cref{app:litsnowball}) was likewise
capped at one generation and 200 forward citations per seed. Third, of the
snowball's kept papers only one is integrated so far; the remainder await
full-text verification and integration. The full session --- queries, retrieved
papers, and screening decisions, now including the snowball's --- is saved and
can be re-exported to reproduce the tables above.
