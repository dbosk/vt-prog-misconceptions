...
\Textcite[Ch.~6]{NCOL} casts scientific discovery as learning on the collective
level. Copernicus' contribution, in this reading, was not the heliocentric view
as such but \emph{centricity}: he opened a dimension of variation in which
heliocentric and geocentric are different features, and once that dimension was
opened, the centricity of many other systems could be considered
\autocite[pp.~155--156]{NCOL}. Darwin likewise opened a dimension --- species
adapt rather than remain invariant --- and Wallace, discerning the same critical
aspects independently, made the same discovery \autocite[pp.~157--160]{NCOL}. A
bug is the programmer's version of such an unknown: something to be discovered
by varying the right aspects, and the patterns of variation in this section are
ones we have observed in our own debugging.
...
Variation theory predicts a further benefit: students taught with patterns of
variation become better at generating such patterns for themselves --- an effect
that surfaces as delayed, or \emph{generative}, learning. As \citeauthor{NCOL}
reports, \textcquote[pp.~227--228]{NCOL}{[t]he outcomes of learning turned out to
be better some time after the learning occasion than immediately after it in
certain classes} --- those taught with patterns of variation --- with no such
effect where patterns were not used \autocite[pp.~227--228]{NCOL}. In the context of
programming this suggests that teaching with variation theory should, over time,
also improve students' debugging, since debugging draws on the same ability to
vary one aspect while holding the others invariant.
